...
/* ========================================================================
   自定义预设（持久化到 localStorage: cf_mvu_group_presets）
   ======================================================================== */
...
function loadCustomPresetsRaw() {
  try {
    const raw = localStorage.getItem('cf_mvu_group_presets');
    const arr = raw ? JSON.parse(raw) : [];
    return Array.isArray(arr) ? arr : [];
  } catch { return []; }
}
function saveCustomPresetsRaw(presets) {
  try {
    localStorage.setItem('cf_mvu_group_presets', JSON.stringify(presets));
  } catch (e) { /* ignore */ }
}

onMounted(() => {
  customPresets.value = loadCustomPresetsRaw();
});
...
function confirmSaveNewPreset() {
  ...
  saveCustomPresetsRaw(customPresets.value);
...
function deleteCustomPreset(presetId) {
  appStore.confirmAction('删除这个自定义预设？', () => {
