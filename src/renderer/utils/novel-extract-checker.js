...
/**
 * 扫描某类提取结果数组里的八股化表达
 * @param {Array} arr 提取结果数组
 * @param {string} type 'character' | 'eventline' | ...
 * @returns {{ total, byType, items }} 总数 + 按八股类型分组 + 命中条目
 */
...
/**
 * 检测某类提取结果里章节号字段为空或可疑的项
 * @returns {Array<{path, expected, current}>} 缺失列表
 */
