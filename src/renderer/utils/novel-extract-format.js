...
export function emptyExtractConfig() {
  return {
    ...
    chaptersPerChunk: 5,
    wordsPerChunk: 10000,
    chapterRangeStart: 1,           // 「只跑前 N 章」起点（默认从 1 开始）
    chapterRangeEnd: 0,             // 0 = 不限制
...
export function chunkNovel(text, opts = {}) {
  ...
  // strategy === 'words' 或 auto fallback
...
/**
 * 把整套提取结果转成世界书条目数组（待注入 cardStore）
 */
export function extractionToWorldEntries(extraction, config) {
  ...
  for (const char of characters.filter(c => c.role === 'major')) {
    entries.push(buildEntry({
      ...
      constant: !isMultiChar,        // 单角色卡蓝灯，多角色卡绿灯
...
function buildEntry({ comment, keys, content, constant, selective, position }) {
  return {
    ...
    enabled: true,
    position,
    insertion_order: 100,             // 朔规则：order 全 100
    extensions: {
...
/**
 * 把 AI 返回的某类提取结果数组规范化（过滤无效项 + 补默认值）
 */
...
export function estimateTotalCalls(chunkCount, selectedTypes, enableR2 = false) {
  ...
  return enableR2 ? baseCalls * 2 + chunkCount : baseCalls;  // R2 双跑 + 合并 1 次
}
...
export function estimateTotalTime(totalCalls, intervalMs = 13000) {
  return Math.ceil(totalCalls * intervalMs / 1000);  // 秒
}

/**
 * 字符数 → 估算 token（中文 1 字 ≈ 1.3 token）
 */
